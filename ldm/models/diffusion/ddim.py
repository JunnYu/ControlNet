...
class DDIMSampler(object):
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True):
        ...
        self.register_buffer('ddim_sigmas', to_paddle(ddim_sigmas))
        self.register_buffer('ddim_alphas', to_paddle(ddim_alphas))
        self.register_buffer('ddim_alphas_prev', to_paddle(ddim_alphas_prev))
        self.register_buffer('ddim_sqrt_one_minus_alphas', to_paddle(np.sqrt(1. - ddim_alphas)))
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            if isinstance(c, dict):
                ...
                for i in range(len(c)):
                    c_in.append(paddle.cat([unconditional_conditioning[i], c[i]]))
            else:
    ...
    def encode(self, x0, c, t_enc, use_original_steps=False, return_intermediates=None,
        ...
        for i in tqdm(range(num_steps), desc='Encoding Image'):
            t = paddle.full((x0.shape[0],), i, device=self.model.device, dtype=paddle.long)
            if unconditional_guidance_scale == 1.:
                noise_pred = self.model.apply_model(x_next, t, c)
            else:
                assert unconditional_conditioning is not None
                e_t_uncond, noise_pred = paddle.chunk(
                    self.model.apply_model(paddle.cat((x_next, x_next)), paddle.cat((t, t)),
                                           paddle.cat((unconditional_conditioning, c))), 2)
    ...
    def decode(self, x_latent, cond, t_start, unconditional_guidance_scale=1.0, unconditional_conditioning=None,
        ...
        for i, step in enumerate(iterator):
            index = total_steps - i - 1
            ts = paddle.full((x_latent.shape[0],), step, device=x_latent.device, dtype=paddle.long)
