...
def get_timestep_embedding(timesteps, embedding_dim):
    ...
    emb = paddle.cat([paddle.sin(emb), paddle.cos(emb)], dim=1)
...
def Normalize(in_channels, num_groups=32):
    return paddle.nn.GroupNorm(num_groups=num_groups, num_channels=in_channels, eps=1e-6, affine=True)
...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, conv_shortcut=False,
        ...
        self.conv1 = paddle.nn.Conv2d(in_channels,
                                     ...
                                     padding=1)
        if temb_channels > 0:
            self.temb_proj = paddle.nn.LinearPT(temb_channels,
                                             out_channels)
    ...
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        if self.use_timestep:
            ...
            self.temb.dense = nn.ModuleList([
                paddle.nn.LinearPT(self.ch,
                                self.temb_ch),
                paddle.nn.LinearPT(self.temb_ch,
                                self.temb_ch),
            ])
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks+1):
                h = self.up[i_level].block[i_block](
                    paddle.cat([h, hs.pop()], dim=1), temb)
...
class LatentRescaler(nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.res_block1:
            x = block(x, None)
        x = paddle.nn.functional.interpolate(x, size=(int(round(x.shape[2]*self.factor)), int(round(x.shape[3]*self.factor))))
        x = self.attn(x)
...
class Resize(nn.Module):
    ...
    def forward(self, x, scale_factor=1.0):
        if scale_factor==1.0:
            ...
            x = paddle.nn.functional.interpolate(x, mode=self.mode, align_corners=False, scale_factor=scale_factor)
        return x
