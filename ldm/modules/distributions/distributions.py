...
class DiagonalGaussianDistribution(object):
    def __init__(self, parameters, deterministic=False):
        ...
        self.mean, self.logvar = paddle.chunk(parameters, 2, dim=1)
    ...
    def kl(self, other=None):
        if self.deterministic:
            ...
            if other is None:
                return 0.5 * paddle.sum(paddle.pow(self.mean, 2)
...
def normal_kl(mean1, logvar1, mean2, logvar2):
    ...
    logvar1, logvar2 = [
        x if isinstance(x, paddle.Tensor) else paddle.tensor(x).to(tensor)
