...
class ContextGuidedBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            joi_feat = paddle.cat([loc, sur], 1)  # the joint feature
...
class CGNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.stem:
            x = layer(x)
        x = self.norm_prelu_0(paddle.cat([x, inp_2x], 1))
        output.append(x)
        ...
        for i, layer in enumerate(self.level1):
            ...
            if i == 0:
                down1 = x
        x = self.norm_prelu_1(paddle.cat([x, down1, inp_4x], 1))
        output.append(x)
        ...
        for i, layer in enumerate(self.level2):
            ...
            if i == 0:
                down2 = x
        x = self.norm_prelu_2(paddle.cat([down2, x], 1))
        output.append(x)
