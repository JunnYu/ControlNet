"""Modified from https://github.com/rwightman/pypaddle-image-
...
class VisionTransformer(nn.Module):
    ...
    A Pypaddle impl of : `An Image is Worth 16x16 Words: Transformers for
    ...
    def __init__(self,
        ...
        self.cls_token = nn.Parameter(paddle.zeros(1, 1, self.embed_dim))
        self.pos_embed = nn.Parameter(
            paddle.zeros(1, self.patch_embed.num_patches + 1, embed_dim))
...
{state_dict["pos_embed"].shape} to {self.pos_embed.shape}')
            ...
            # https://github.com/rwightman/pypaddle-image-models/blob/master/timm/models/vision_transformer.py#L353  # noqa: E501
    ...
    def _pos_embeding(self, img, patched_img, pos_embed):
        ...
        Resize the pos_embed, if the input image size doesn't match
            the training size.
        Args:
            img (paddle.Tensor): The inference image tensor, the shape
                must be [B, C, H, W].
            patched_img (paddle.Tensor): The patched image, it should be
                shape of [B, L1, C].
            pos_embed (paddle.Tensor): The pos_embed weighs, it should be
                shape of [B, L2, c].
        Return:
            paddle.Tensor: The pos encoded image feature.
        """
    ...
    def resize_pos_embed(pos_embed, input_shpae, pos_shape, patch_size, mode):
        ...
        Args:
            pos_embed (paddle.Tensor): pos_embed weights.
            input_shpae (tuple): Tuple for (input_h, intput_w).
            pos_shape (tuple): Tuple for (pos_h, pos_w).
            patch_size (int): Patch size.
        Return:
            paddle.Tensor: The resized pos_embed of shape [B, L_new, C]
        """
        ...
        pos_embed_weight = paddle.flatten(pos_embed_weight, 2).transpose(1, 2)
        pos_embed = paddle.cat((cls_token_weight, pos_embed_weight), dim=1)
        return pos_embed
    ...
    def forward(self, inputs):
        ...
        x = paddle.cat((cls_tokens, x), dim=1)
