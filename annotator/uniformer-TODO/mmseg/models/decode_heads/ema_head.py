...
class EMAModule(nn.Module):
    ...
    def __init__(self, channels, num_bases, num_stages, momentum):
        ...
        bases = paddle.zeros(1, channels, self.num_bases)
    ...
    def forward(self, feats):
        ...
        with paddle.no_grad():
            for i in range(self.num_stages):
                ...
                attention = paddle.einsum('bcn,bck->bnk', feats, bases)
                attention = F.softmax(attention, dim=2)
                # l1 norm
                attention_normed = F.normalize(attention, dim=1, p=1)
                # [batch_size, channels, num_bases]
                bases = paddle.einsum('bcn,bnk->bck', feats, attention_normed)
                # l2 norm
                bases = F.normalize(bases, dim=1, p=2)

        feats_recon = paddle.einsum('bck,bnk->bcn', bases, attention)
...
class EMAHead(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        if self.concat_input:
            output = self.conv_cat(paddle.cat([x, output], dim=1))
