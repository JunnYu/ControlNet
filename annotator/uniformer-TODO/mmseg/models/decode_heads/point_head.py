...
def calculate_uncertainty(seg_logits):
    ...
    """
    top2_scores = paddle.topk(seg_logits, k=2, dim=1)[0]
...
class PointHead(BaseCascadeDecodeHead):
    ...
    def forward(self, fine_grained_point_feats, coarse_point_feats):
        x = paddle.cat([fine_grained_point_feats, coarse_point_feats], dim=1)
        for fc in self.fcs:
            x = fc(x)
            if self.coarse_pred_each_layer:
                x = paddle.cat((x, coarse_point_feats), dim=1)
    ...
    def _get_fine_grained_point_feats(self, x, points):
        ...
        if len(fine_grained_feats_list) > 1:
            fine_grained_feats = paddle.cat(fine_grained_feats_list, dim=1)
        else:
    ...
    def get_points_train(self, seg_logits, uncertainty_func, cfg):
        ...
        shift = num_sampled * paddle.arange(
            batch_size, dtype=paddle.long, device=seg_logits.device)
        idx += shift[:, None]
        point_coords = point_coords.view(-1, 2)[idx.view(-1), :].view(
            batch_size, num_uncertain_points, 2)
        if num_random_points > 0:
            rand_point_coords = paddle.rand(
                batch_size, num_random_points, 2, device=seg_logits.device)
            point_coords = paddle.cat((point_coords, rand_point_coords), dim=1)
        return point_coords
