...
class PPMConcat(nn.ModuleList):
    ...
    def forward(self, feats):
        ...
        concat_outs = paddle.cat(ppm_outs, dim=2)
        return concat_outs
...
class AFNB(nn.Module):
    ...
    def forward(self, low_feats, high_feats):
        ...
        context = paddle.stack(priors, dim=0).sum(dim=0)
        output = self.bottleneck(paddle.cat([context, high_feats], 1))
        return output
...
class APNB(nn.Module):
    ...
    def forward(self, feats):
        ...
        context = paddle.stack(priors, dim=0).sum(dim=0)
        output = self.bottleneck(paddle.cat([context, feats], 1))
        return output
