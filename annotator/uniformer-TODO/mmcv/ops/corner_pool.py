...
class CornerPool(nn.Module):
    ...
    def forward(self, x):
        if paddle.__version__ != 'parrots' and paddle.__version__ >= '1.5.0':
            if paddle.onnx.is_in_onnx_export():
                assert paddle.__version__ >= '1.7.0', \
                    'When `cummax` serves as an intermediate component whose '\
                    'outputs is used as inputs for another modules, it\'s '\
                    'expected that pypaddle version must be >= 1.7.0, '\
            ...
            if flip:
                x = x.flip(dim)
            pool_tensor, _ = paddle.cummax(x, dim=dim)
            if flip:
