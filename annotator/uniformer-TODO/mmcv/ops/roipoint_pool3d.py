...
class RoIPointPool3d(nn.Module):
    ...
    def forward(self, points, point_features, boxes3d):
        """
        Args:
            points (paddle.Tensor): Input points whose shape is (B, N, C).
            point_features (paddle.Tensor): Features of input points whose shape
                is (B, N, C).
            boxes3d (B, M, 7), Input bounding boxes whose shape is (B, M, 7).

        Returns:
            pooled_features (paddle.Tensor): The output pooled features whose
                shape is (B, M, 512, 3 + C).
            pooled_empty_flag (paddle.Tensor): Empty flag whose shape is (B, M).
        """
