...
from paddle.nn.modules.module import Module
...
class SyncBatchNormFunction(Function):
    ...
    def forward(self, input, running_mean, running_var, weight, bias, momentum,
        ...
        assert isinstance(
                   input, (paddle.HalfTensor, paddle.FloatTensor,
                           paddle.cuda.HalfTensor, paddle.cuda.FloatTensor)), \
        ...
        # ``paddle.empty()`` does not guarantee that
        mean = paddle.zeros(
            num_channels, dtype=paddle.float, device=input3d.device)
        var = paddle.zeros(
            num_channels, dtype=paddle.float, device=input3d.device)
        norm = paddle.zeros_like(
            input3d, dtype=paddle.float, device=input3d.device)
        std = paddle.zeros(
            num_channels, dtype=paddle.float, device=input3d.device)
        ...
        if batch_size > 0:
            ...
            batch_flag = paddle.ones([1], device=mean.device, dtype=mean.dtype)
        else:
            # skip updating mean and leave it as zeros when the input is empty
            batch_flag = paddle.zeros([1], device=mean.device, dtype=mean.dtype)

        # synchronize mean and the batch flag
        vec = paddle.cat([mean, batch_flag])
    ...
    def backward(self, grad_output):
        ...
        grad_bias = paddle.zeros_like(weight)
        grad_input = paddle.zeros_like(grad_output)
...
class SyncBatchNorm(Module):
    ...
    def __init__(self,
        ...
        self.stats_mode = stats_mode
        if self.affine:
            self.weight = Parameter(paddle.Tensor(num_features))
            self.bias = Parameter(paddle.Tensor(num_features))
        else:
        ...
        if self.track_running_stats:
            ...
            self.register_buffer('num_batches_tracked',
                                 paddle.to_tensor(0, dtype=paddle.long))
        else:
    ...
    def reset_parameters(self):
        ...
        if self.affine:
            self.weight.data.uniform_()  # pypaddle use ones_()
