    ...
    def forward(self, points_xyz, center_xyz, features=None):
            ...
            for i_batch in range(idx.shape[0]):
                for i_region in range(idx.shape[1]):
                    ...
                    sample_ind = paddle.randint(
                        ...
                        dtype=paddle.long)
                    all_ind = paddle.cat((unique_ind, unique_ind[sample_ind]))
        ...
        if features is not None:
            ...
            if self.use_xyz:
                ...
                new_features = paddle.cat([grouped_xyz_diff, grouped_features],
                                         dim=1)
            else:
...
class GroupAll(nn.Module):
    ...
    def forward(self,
        ...
        if features is not None:
            ...
            if self.use_xyz:
                # (B, 3 + C, 1, N)
                new_features = paddle.cat([grouped_xyz, grouped_features],
                                         dim=1)
            else:
                new_features = grouped_features
        else:
...
class GroupingOperation(Function):
    ...
    def forward(ctx, features: paddle.Tensor,
        ...
        _, C, N = features.size()
        output = paddle.cuda.FloatTensor(B, C, nfeatures, nsample)
    ...
    @staticmethod
    def backward(ctx,
                 grad_out: paddle.Tensor) -> Tuple[paddle.Tensor, paddle.Tensor]:
        """
        Args:
        ...
        grad_features = paddle.cuda.FloatTensor(B, C, N).zero_()
