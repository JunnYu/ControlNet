...
def boxes_iou_bev(boxes_a, boxes_b):
    ...
    Args:
        boxes_a (paddle.Tensor): Input boxes a with shape (M, 5).
        boxes_b (paddle.Tensor): Input boxes b with shape (N, 5).

    Returns:
        ans_iou (paddle.Tensor): IoU result with shape (M, N).
    """
    ans_iou = boxes_a.new_zeros(
        paddle.Size((boxes_a.shape[0], boxes_b.shape[0])))
...
def nms_bev(boxes, scores, thresh, pre_max_size=None, post_max_size=None):
    ...
    keep = paddle.zeros(boxes.size(0), dtype=paddle.long)
...
def nms_normal_bev(boxes, scores, thresh):
    ...
    Args:
        boxes (paddle.Tensor): Input boxes with shape (N, 5).
        scores (paddle.Tensor): Scores of predicted boxes with shape (N).
        thresh (float): Overlap threshold of NMS.

    Returns:
        paddle.Tensor: Remaining indices with scores in descending order.
    """
