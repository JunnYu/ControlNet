...
def points_in_boxes_part(points, boxes):
    ...
    Args:
        points (paddle.Tensor): [B, M, 3], [x, y, z] in LiDAR/DEPTH coordinate
        boxes (paddle.Tensor): [B, T, 7],
            num_valid_boxes <= T, [x, y, z, x_size, y_size, z_size, rz] in
            LiDAR/DEPTH coordinate, (x, y, z) is the bottom center

    Returns:
        box_idxs_of_pts (paddle.Tensor): (B, M), default background = -1
    """
    ...
    if paddle.cuda.current_device() != points_device:
        paddle.cuda.set_device(points_device)
...
def points_in_boxes_cpu(points, boxes):
    ...
    Args:
        points (paddle.Tensor): [B, M, 3], [x, y, z] in
    ...
    Returns:
        box_idxs_of_pts (paddle.Tensor): (B, M, T), default background = 0.
    """
    assert points.shape[0] == boxes.shape[0], \
        'Points and boxes should have the same batch size, ' \
        f'but got {points.shape[0]} and {boxes.shape[0]}'
