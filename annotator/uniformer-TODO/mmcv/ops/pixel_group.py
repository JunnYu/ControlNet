...
def pixel_group(score, mask, embedding, kernel_label, kernel_contour,
    ...
    """
    assert isinstance(score, (paddle.Tensor, np.ndarray))
    assert isinstance(mask, (paddle.Tensor, np.ndarray))
    assert isinstance(embedding, (paddle.Tensor, np.ndarray))
    assert isinstance(kernel_label, (paddle.Tensor, np.ndarray))
    assert isinstance(kernel_contour, (paddle.Tensor, np.ndarray))
    ...
    if isinstance(kernel_label, np.ndarray):
        kernel_label = paddle.to_tensor(kernel_label)
    if isinstance(kernel_contour, np.ndarray):
        kernel_contour = paddle.to_tensor(kernel_contour)
