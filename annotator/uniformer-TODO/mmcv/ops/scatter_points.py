...
class _DynamicScatter(Function):
    ...
    def forward(ctx, feats, coors, reduce_type='max'):
        ...
        Args:
            feats (paddle.Tensor): [N, C]. Points features to be reduced
                into voxels.
            coors (paddle.Tensor): [N, ndim]. Corresponding voxel coordinates
                (specifically multi-dim voxel index) of each points.
            reduce_type (str, optional): Reduce op. support 'max', 'sum' and
                'mean'. Default: 'max'.

        Returns:
            voxel_feats (paddle.Tensor): [M, C]. Reduced features, input
                features that shares the same voxel coordinates are reduced to
                one row.
            voxel_coors (paddle.Tensor): [M, ndim]. Voxel coordinates.
        """
...
class DynamicScatter(nn.Module):
    ...
    def forward_single(self, points, coors):
        ...
        Returns:
            voxel_feats (paddle.Tensor): Reduced features, input features that
                shares the same voxel coordinates are reduced to one row.
            voxel_coors (paddle.Tensor): Voxel coordinates.
        """
    ...
    def forward(self, points, coors):
        ...
        if coors.size(-1) == 3:
            ...
            for i in range(batch_size):
                ...
                voxels.append(voxel)
            features = paddle.cat(voxels, dim=0)
            feature_coors = paddle.cat(voxel_coors, dim=0)
