...
def calc_square_dist(point_feat_a, point_feat_b, norm=True):
    ...
    # [bs, n, 1]
    a_square = paddle.sum(point_feat_a.unsqueeze(dim=2).pow(2), dim=-1)
    # [bs, 1, m]
    b_square = paddle.sum(point_feat_b.unsqueeze(dim=1).pow(2), dim=-1)

    corr_matrix = paddle.matmul(point_feat_a, point_feat_b.transpose(1, 2))
...
class PointsSampler(nn.Module):
    ...
    def forward(self, points_xyz, features):
        ...
        indices = paddle.cat(indices, dim=1)
...
class FFPSSampler(nn.Module):
    ...
    def forward(self, points, features, npoint):
        ...
        features_for_fps = paddle.cat([points, features.transpose(1, 2)], dim=2)
...
class FSSampler(nn.Module):
    ...
    def forward(self, points, features, npoint):
        ...
        fps_idx = paddle.cat([fps_idx_ffps, fps_idx_dfps], dim=1)
        return fps_idx
