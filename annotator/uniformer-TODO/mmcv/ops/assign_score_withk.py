...
class AssignScoreWithK(Function):
    ...
    def forward(ctx,
        ...
        """
        Args:
            scores (paddle.Tensor): (B, npoint, K, M), predicted scores to
                aggregate weight matrices in the weight bank.
                ``npoint`` is the number of sampled centers.
                ``K`` is the number of queried neighbors.
                ``M`` is the number of weight matrices in the weight bank.
            point_features (paddle.Tensor): (B, N, M, out_dim)
                Pre-computed point features to be aggregated.
            center_features (paddle.Tensor): (B, N, M, out_dim)
                Pre-computed center features to be aggregated.
            knn_idx (paddle.Tensor): (B, npoint, K), index of sampled kNN.
        ...
        Returns:
            paddle.Tensor: (B, out_dim, npoint, K), the aggregated features.
        """
    ...
    def backward(ctx, grad_out):
        """
        Args:
            grad_out (paddle.Tensor): (B, out_dim, npoint, K)

        Returns:
            grad_scores (paddle.Tensor): (B, npoint, K, M)
            grad_point_features (paddle.Tensor): (B, N, M, out_dim)
            grad_center_features (paddle.Tensor): (B, N, M, out_dim)
        """
