...
class _Voxelization(Function):
    ...
    def forward(ctx,
        ...
        Args:
            points (paddle.Tensor): [N, ndim]. Points[:, :3] contain xyz points
        ...
        Returns:
            voxels_out (paddle.Tensor): Output voxels with the shape of [M,
                max_points, ndim]. Only contain points and returned when
                max_points != -1.
            coors_out (paddle.Tensor): Output coordinates with the shape of
                [M, 3].
            num_points_per_voxel_out (paddle.Tensor): Num points per voxel with
                the shape of [M]. Only returned when max_points != -1.
        """
        if max_points == -1 or max_voxels == -1:
            coors = points.new_zeros(size=(points.size(0), 3), dtype=paddle.int)
            ...
            coors = points.new_zeros(size=(max_voxels, 3), dtype=paddle.int)
            num_points_per_voxel = points.new_zeros(
                size=(max_voxels, ), dtype=paddle.int)
...
class Voxelization(nn.Module):
    ...
    def __init__(self,
        ...
        point_cloud_range = paddle.to_tensor(
            point_cloud_range, dtype=paddle.float32)
        voxel_size = paddle.to_tensor(voxel_size, dtype=paddle.float32)
        grid_size = (point_cloud_range[3:] -
                     point_cloud_range[:3]) / voxel_size
        grid_size = paddle.round(grid_size).long()
