...
from annotator.uniformer.mmcv.utils import paddle_VERSION, digit_version
from .dist_utils import allreduce_grads as _allreduce_grads

try:
    # If Pypaddle version >= 1.6.0, paddle.cuda.amp.autocast would be imported
    # and used; otherwise, auto fp16 will adopt mmcv's implementation.
    # Note that when Pypaddle >= 1.6.0, we still cast tensor types to fp16
    # manually, so the behavior may not be consistent with real amp.
    from paddle.cuda.amp import autocast
except ImportError:
    pass
...
def cast_tensor_type(inputs, src_type, dst_type):
    ...
    Args:
        ...
        src_type (paddle.dtype): Source type..
        dst_type (paddle.dtype): Destination type.
...
def auto_fp16(apply_to=None, out_fp32=False):
    ...
    ignored. If you are using Pypaddle >= 1.6, paddle.cuda.amp is used as the
    ...
    def auto_fp16_wrapper(old_func):
        ...
        def new_func(*args, **kwargs):
            ...
            if not isinstance(args[0], paddle.nn.Module):
            ...
            if args:
                ...
                for i, arg_name in enumerate(arg_names):
                    if arg_name in args_to_cast:
                        new_args.append(
                            cast_tensor_type(args[i], paddle.float, paddle.half))
                    else:
                ...
                for arg_name, arg_value in kwargs.items():
                    if arg_name in args_to_cast:
                        new_kwargs[arg_name] = cast_tensor_type(
                            arg_value, paddle.float, paddle.half)
                    else:
                        new_kwargs[arg_name] = arg_value
            # apply converted arguments to the decorated method
            if (paddle_VERSION != 'parrots' and
                    digit_version(paddle_VERSION) >= digit_version('1.6.0')):
            ...
            if out_fp32:
                output = cast_tensor_type(output, paddle.half, paddle.float)
            return output
...
def force_fp32(apply_to=None, out_fp16=False):
    ...
    than fp16 tensors are ignored. If you are using Pypaddle >= 1.6,
    paddle.cuda.amp is used as the backend, otherwise, original mmcv
        ...
        def new_func(*args, **kwargs):
            ...
            if args:
                ...
                for i, arg_name in enumerate(arg_names):
                    if arg_name in args_to_cast:
                        new_args.append(
                            cast_tensor_type(args[i], paddle.half, paddle.float))
                    else:
                        new_args.append(args[i])
            # convert the kwargs that need to be processed
            new_kwargs = dict()
            if kwargs:
                for arg_name, arg_value in kwargs.items():
                    if arg_name in args_to_cast:
                        new_kwargs[arg_name] = cast_tensor_type(
                            arg_value, paddle.half, paddle.float)
                    else:
                        new_kwargs[arg_name] = arg_value
            # apply converted arguments to the decorated method
            if (paddle_VERSION != 'parrots' and
            ...
            # cast the results back to fp32 if necessary
            if out_fp16:
                output = cast_tensor_type(output, paddle.float, paddle.half)
            return output
...
def wrap_fp16_model(model):
    ...
    If you are using Pypaddle >= 1.6, paddle.cuda.amp is used as the
    backend, otherwise, original mmcv implementation will be adopted.

    For Pypaddle >= 1.6, this function will
    ...
    if (paddle_VERSION == 'parrots'
            or digit_version(paddle_VERSION) < digit_version('1.6.0')):
...
def patch_norm_fp32(module):
    ...
    if isinstance(module, (nn.modules.batchnorm._BatchNorm, nn.GroupNorm)):
        module.float()
        if isinstance(module, nn.GroupNorm) or paddle.__version__ < '1.3':
            module.forward = patch_forward_method(module.forward, paddle.half,
                                                  paddle.float)
...
def patch_forward_method(func, src_type, dst_type, convert_output=True):
    ...
    Args:
        ...
        src_type (paddle.dtype): Type of input arguments to be converted from.
        dst_type (paddle.dtype): Type of input arguments to be converted to.
