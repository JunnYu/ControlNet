...
class ProfilerHook(Hook):
    ...
    Pypaddle Profiler is a tool that allows the collection of the performance
    metrics during the training. More details on Profiler can be found at
    https://pypaddle.org/docs/1.8.1/profiler.html#paddle.profiler.profile
    ...
    def __init__(self,
        ...
        try:
            from paddle import profiler  # paddle version >= 1.8.1
        except ImportError:
            raise ImportError('profiler is the new feature of paddle1.8.1, '
                              f'but your version is {paddle.__version__}')
    ...
    def before_run(self, runner):
        ...
        if callable(self.on_trace_ready):  # handler
            ...
            if trace_type == 'log_trace':
                ...
                try:
                    import paddle_tb_profiler  # noqa: F401
                except ImportError:
                    raise ImportError('please run "pip install '
                                      'paddle-tb-profiler" to install '
                                      'paddle_tb_profiler')
                _on_trace_ready = paddle.profiler.tensorboard_trace_handler(
                    **trace_cfg)
            else:
        ...
        self.profiler = paddle.profiler.profile(
